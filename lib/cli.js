...
const getEmployees = async () => {
    ...
    const map = employees[0].map(({ id, name }) => ({ name: name, value: {id: id, name: name} }))
    return map;
};
...
const updateRolePrompt = async () => {
    ...
    return inquirer
        ...
        .then((answers) => {
            updateEmployee(answers.employee.id, answers.role, answers.manager);
            console.log(`Successfully updated ${answers.employee.name}'s role in company database.`);
            starterPrompt();
        });
};
